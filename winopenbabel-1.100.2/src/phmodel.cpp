...
{
	printf("OBPhModel::AssignSeedPartialCharge\n");
	if (!_init) Init();
	printf("OBPhModel::AssignSeedPartialCharge 2\n");
	
	mol.SetPartialChargesPerceived();
	printf("OBPhModel::AssignSeedPartialCharge 3\n");
	if (!mol.AutomaticPartialCharge()) return;
	printf("OBPhModel::AssignSeedPartialCharge 4\n");
	vector<pair<OBSmartsPattern*,vector<double> > >::iterator i;
	for (i = _vschrg.begin();i != _vschrg.end();i++)
	{
		printf("OBPhModel::AssignSeedPartialCharge 5\n");
		if (i->first->Match(mol))
		{
		printf("OBPhModel::AssignSeedPartialCharge 6\n");
			_mlist = i->first->GetUMapList();
			unsigned int k;
			vector<vector<int> >::iterator j;
			for (j = _mlist.begin();j != _mlist.end();j++)
			{
		printf("OBPhModel::AssignSeedPartialCharge 7\n");
				for (k = 0;k < j->size();k++)
				{
		printf("OBPhModel::AssignSeedPartialCharge 8\n");
					mol.GetAtom((*j)[k])->SetPartialCharge(i->second[k]);
		printf("OBPhModel::AssignSeedPartialCharge 9\n");
				}
		printf("OBPhModel::AssignSeedPartialCharge 10\n");
			}
		printf("OBPhModel::AssignSeedPartialCharge 11\n");
		}
		printf("OBPhModel::AssignSeedPartialCharge 12\n");
	}
		printf("OBPhModel::AssignSeedPartialCharge end\n");
}
