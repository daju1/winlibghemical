...
bool WriteGhemical(ostream &ofs,OBMol &mol)
{
	...
	printf("WriteGhemical 4\n");
    
	// Calculated charges
	ofs << "!Charges" << endl;
	if (IDOK == MessageBox(0, "atom->GetPartialCharge ? ", "WriteGhemical()", MB_OKCANCEL))
	{
		for(i = 1;i <= mol.NumAtoms(); i++)
		{
			printf("0 i = %d\n", i);
	...
	else
	{
		for(i = 1;i <= mol.NumAtoms(); i++)
		{
			...
			sprintf(buffer,"%d %f",
				i-1,
				0.0/*atom->GetPartialCharge()*/);
			printf("2 i = %d\n", i);
            
			ofs << buffer << endl;
			printf("3 i = %d\n", i);
		}
		printf("WriteGhemical 5\n");
        
		ofs << "!End" << endl;
		printf("WriteGhemical end\n");
		return(true);
	}
}
