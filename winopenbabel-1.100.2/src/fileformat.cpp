...
bool OBFileFormat::ReadMolecule(istream &ifs, OBMol &mol, const char *title)
{
  ...
  //printf("OBFileFormat::ReadMolecule %s mol.GetInputType() = %d\n\n", title, mol.GetInputType());
  ...
  switch(mol.GetInputType())
    ...
    case GHEMICAL:  
		result = ReadGhemical(ifs,mol,title);	
//		printf("ReadGhemical result = %d", result);
		break; 
  ...
  printf("OBFileFormat::ReadMolecule end\n");
...
{
//  AfxMessageBox("WriteMolecule 1");
printf("OBFileFormat::WriteMolecule mol.GetOutputType() = %d dimension %s options %s\n\n", mol.GetOutputType(), dimension, options);
...
printf("OBFileFormat::WriteMoleculeend\n\n");
