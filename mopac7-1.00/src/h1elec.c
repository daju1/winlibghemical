/* h1elec.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int h1elec_(ni, nj, xi, xj, smat)
integer *ni, *nj;
doublereal *xi, *xj, *smat;
{
    /* Initialized data */
    ...
    /* Local variables */
    extern /* Subroutine */ int diat_();
#define lims ((integer *)&ucell_1)
    static doublereal xjuc[3];
    static integer i__, j, k, l, m, nbond, norbi, norbj;
    static doublereal sbits[81]	/* was [9][9] */, bi[9], bj[9];
    static integer ii;
...
/* Computing 2nd power */
	d__3 = xi[3] - xj[3];
	if (sqrt(d__1 * d__1 + d__2 * d__2 + d__3 * d__3) > (float)1.8) {
	    for (i__ = 1; i__ <= 9; ++i__) {
		for (j = 1; j <= 9; ++j) {
/* L10: */
		    smat[i__ + j * 9] = 0.;
