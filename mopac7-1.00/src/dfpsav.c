/* dfpsav.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int dfpsav_(totime, xparam, gd, xlast, funct1, mdfp, xdfp)
doublereal *totime, *xparam, *gd, *xlast, *funct1;
integer *mdfp;
doublereal *xdfp;
{
    /* Initialized data */
    ...
    /* Builtin functions */
    integer f_open(), f_rew(), s_wsfe(), e_wsfe(), i_indx(), do_fio();
    /* Subroutine */ int s_stop();
    integer s_wsue(), do_uio(), e_wsue(), f_clos(), s_rsue(), e_rsue();
...
',//       10X,' HERE IS A DATA-FILE FILES THAT MIGHT BE SUITABLE',/       1\
0X,' FOR RESTARTING THE CALCULATION',///)", 0 };
...
DUMPED TO DISK',  /10X,'   RESTART IT USING THE MAGIC WORD \"RESTART\"')", 0 }
	    ;
