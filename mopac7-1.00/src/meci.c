/* meci.f -- translated by f2c (version 19991025).
...
doublereal meci_(eigs, coeff)
doublereal *eigs, *coeff;
{
    /* Initialized data */

    static integer icalcn = 0;
    static char tspin[8*7+1] = "SINGLET DOUBLET TRIPLET QUARTET QUINTET SEXT\
ET  SEPTET  ";
...
VALUE OF S**2  S FROM S**2=S(S+1)\002,//)";
    ...
    integer f_rew(), s_rsfe(), e_rsfe(), s_cmp();
    /* Subroutine */ int s_copy();
    double sqrt();
    integer i_dnnt(), s_wsle(), e_wsle(), do_lio();

    /* Local variables */
    static doublereal eiga[8];
    static integer lima, limb;
    static char line[80];
    static logical doub;
    static integer mdim;
    extern /* Subroutine */ int ijkl_();
    static logical sing;
    extern /* Subroutine */ int perm_();
    static logical quar;
    ...
    static integer j, l;
    extern doublereal reada_(), diagi_();
    extern /* Subroutine */ int mecih_();
    ...
    static integer ndown, izero, i1, j1, lroot, kroot, iroot;
    static doublereal smult;
    static logical lspin1, first1;
    static integer ii, ne, ji, li, in, il, iu, nperma[384]	/* was [8][48]
	     */, npermb[384]	/* was [8][48] */;
    static logical bigprt;
    static integer ndoubl;
    static doublereal xx;
    static integer labsiz, maxvec, natoms;
    extern /* Subroutine */ int vecprt_();
    static integer iofset;
    extern /* Subroutine */ int matout_();
...
/* L420: */
	...
	y = (sqrt(x * 4. + 1.) - 1.) * .5;
	if ((d__1 = smult - x, abs(d__1)) < (float).01) {
	    ++iroot;
	    if (iroot == lroot) {
		kroot = spqr_1.i__;
