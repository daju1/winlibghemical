/* wrtkey.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int wrtkey_(keywrd, keywrd_len)
char *keywrd;
ftnlen keywrd_len;
{
    /* Initialized data */
...
LEVELS USED IN A \002,/,\002 *             C.I. INVOLVING \002,i2,\002 M.O.'S\
\002)";
...
,\002 SPECIFIED\002)";
    ...
    integer s_wsfe(), e_wsfe(), i_indx(), do_fio(), i_dnnt();
...
WITH ',             'GROUP')", 0 };
    ...
    static cilist io___180 = { 0, 6, 0, "(//10X,                            \
                ' UHF USED WITH EITHER BIRAD, EXCITED OR C.I. ')", 0 };
    ...
    static cilist io___186 = { 0, 6, 0, "(//10X,                            \
                   ' ONLY ONE OF MINDO, MNDO, AM1 AND PM3 ALLOWED')", 0 };
...
ON HAS BEEN ',   'SPECIFIED',/10X,                                          \
     'CONFLICT MUST BE RESOLVED BEFORE JOB WILL RUN')", 0 };
...
logical myword_(keywrd, testwd, keywrd_len, testwd_len)
char *keywrd, *testwd;
ftnlen keywrd_len;
ftnlen testwd_len;
{
