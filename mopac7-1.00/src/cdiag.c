/* cdiag.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int cdiag_(a, value, vec, n, need)
complex *a;
real *value;
complex *vec;
integer *n, *need;
{
    ...
    /* Local variables */
    extern /* Subroutine */ int ec08c_(), me08a_(), sort_();
    static real h__;
    static integer i__, k, l;
    static real s;
    static complex w[6000];
    extern /* Complex */ VOID fm06as_();
...
/* FIND THE EIGENVALUES AND EIGENVECTORS OF THE TRI-DIAGONAL MATRIX */
    ec08c_(w, &w[*n], &value[1], &vec[vec_offset], n, &iv, &w[*n * 2]);
    if (*need == 0) {
	goto L50;
    }
    if (*n < 2) {
...
/* Subroutine */ int ea08c_(a, b, value, vec, m, iv, w)
real *a, *b, *value, *vec;
integer *m, *iv;
real *w;
{
...
G NOW\002)";
    ...
    /* Fortran I/O blocks */
    static cilist io___47 = { 0, 6, 0, fmt_90, 0 };
...
/*     EACH QR ITERATION IS PERFORMED OF ROWS AND COLUMNS N1 TO N2 */
	...
	for (iter = 1; iter <= 20; ++iter) {
	    bb = (value[n2] - value[n2 - 1]) * (float).5;
	    cc = w[n2] * w[n2];
	    a22 = value[n2];
	    if (cc != (float)0.) {
		a22 += cc / (bb + r_sign(&c_b13, &bb) * sqrt(bb * bb + cc));
	    }
	    i__2 = n2;
...
/* Computing 2nd power */
	    ...
	    xx += r__1 * r__1;
/* L110: */
	    xax += vec[ki] * (b[i__] * (float)2. * vec[ki - 1] + a[i__] * vec[
		    ki]);
	}
/* L120: */
	value[j] = xax / xx;
...
/* Subroutine */ int ea09c_(a, b, value, m, off)
real *a, *b, *value;
integer *m;
real *off;
{
    ...
    /* Fortran I/O blocks */
    static cilist io___75 = { 0, 6, 0, fmt_90, 0 };
...
/* Subroutine */ int ec08c_(a, b, value, vec, n, iv, w)
complex *a, *b;
real *value, *vec;
integer *n, *iv;
real *w;
...
/* Complex */ VOID fm06as_( ret_val, n, a, ia, b, ib)
complex * ret_val;
integer *n;
complex *a;
integer *ia;
...
/* Complex */ VOID fm06bs_( ret_val, n, a, ia, b, ib)
complex * ret_val;
integer *n;
complex *a;
integer *ia;
...
/* Subroutine */ int me08a_(a, alpha, beta, n, ia, q)
...
complex *q;
{
    ...
    extern /* Complex */ VOID fm06as_(), fm06bs_();
...
/* L50: */
	    i__4 = *n - k + 1;
	    i__1 = *ia << 1;
	    me08b_(&a[k + k * a_dim1], &q[k], &a[i__ + k * a_dim1], &i__4, &
		    i__1);
	}
L60:
	;
...
/* Subroutine */ int me08b_(a, q, b, n, ia)
real *a, *q, *b;
integer *n, *ia;
{
    /* System generated locals */
...
/* Subroutine */ int sort_(val, vec, n)
real *val;
complex *vec;
integer *n;
{
