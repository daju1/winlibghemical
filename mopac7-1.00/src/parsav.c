/* parsav.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int parsav_(mode, n, m)
integer *mode, *n, *m;
{
    ...
    /* Builtin functions */
    integer f_open(), f_rew(), s_rsue(), do_uio(), e_rsue(), i_indx(), s_wsfe(
	    ), e_wsfe(), s_wsue(), e_wsue(), f_clos();
...
VARIABLES',//)", 0 };
    static cilist io___11 = { 0, 6, 0, "(//10X,                             \
                  'TO RESTART CALCULATION USE THE KEYWORD \"RESTART\".')", 0 }
	    ;
