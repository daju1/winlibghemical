/* powsav.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int powsav_(hess, grad, xparam, pmat, iloop, bmat, ipow)
doublereal *hess, *grad, *xparam, *pmat;
integer *iloop;
doublereal *bmat;
integer *ipow;
{
    ...
    /* Builtin functions */
    integer f_open(), f_rew(), s_wsfe(), e_wsfe();
    double sqrt();
    integer do_fio(), s_wsue(), do_uio(), e_wsue(), i_indx(), f_clos(), 
	    s_rsue(), e_rsue();
...
UMPED TO DISK',/10X,'   RESTART IT USING THE KEY-WORD \"RESTART\"')", 0 };
