...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	if (rm == Draw || rm == Erase || rm == Select || rm == Measure || rm == MolAxis)
	{
...
{
	printf("name_vector[%d] = %d %x\n", iii, name_vector[iii], name_vector[iii]);
}*/		
		// "draw"- and "erase"-events are always forwarded to the prj-object:
		// "draw"- and "erase"-events are always forwarded to the prj-object:
		// "draw"- and "erase"-events are always forwarded to the prj-object:
		
		if (rm == MolAxis) gv->GetProject()->MolAxisEvent(gv, name_vector);
