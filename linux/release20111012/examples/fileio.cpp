...
bool ReadGPR_OLD(project & prj, istream & file, bool, bool)
{
	...
	bool result = app->Question(message);
...
cout << "model::ReadStream_OLD() : starting file input." << endl;
	...
	while (file.peek() != 'E')
	{
		...
		prj.AddAtom_lg(newatom); tmp1.push_back(& prj.GetLastAtom());
...
if (n1 != ar_vector.size()) cout << "WARNING : bad atom id numbers!!!" << endl;
				...
				if (selected) newatom.flags |= ATOMFLAG_USER_SELECTED;
				
				prj.AddAtom_lg(newatom);
...
cout << "model::ReadStream() : starting file input." << endl;
	...
	prj.ReplaceCurrentSetup(new setup1_mm(& prj));
...
if (n1 >= ar_vector.size()) { cout << "ERROR : invalid atom id number!!!" << endl; exit(EXIT_FAILURE); }
	...
	if (!newsu && !strcmp(setup1_qm::static_GetClassName(), su_cn)) newsu = new setup1_qm(& prj);
	if (!newsu && !strcmp(setup1_mm::static_GetClassName(), su_cn)) newsu = new setup1_mm(& prj);
	//if (!newsu && !strcmp(setup1_sf::static_GetClassName(), su_cn)) newsu = new setup1_sf(& prj, false);
	
	if (!newsu && !strcmp(setup2_qm_mm::static_GetClassName(), su_cn)) newsu = new setup2_qm_mm(& prj);
	
	if (!newsu)
	{
		prj.WarningMessage("did not find a matching setup");
		newsu = new setup1_mm(& prj);
	}
	
	prj.ReplaceCurrentSetup(newsu);
	...
	prj.GetCurrentSetup()->SetCurrEngIndex(index);
...
if (n1 != ar_vector.size()) cout << "WARNING : atom id number mismatch!!!" << endl;
				...
				newatom.flags = (n4 & 0xFF);	// skip internal...
				if (selected) newatom.flags |= ATOMFLAG_USER_SELECTED;
...
//////////////////////////////////////////////////////////////////////////
/*prj.Message("\
PLEASE NOTE : This file format is EXPERIMENTAL!\n\n\
This file format is NOT compatible with OpenBabel-1.100.2.\n\
Please use the File/Export to generate OpenBabel-compatible files.");*/
//////////////////////////////////////////////////////////////////////////
...
// the above only saves the most primitive setup information; need to add !Setup section later???
	...
	file << prj.GetCurrentSetup()->GetClassName_lg() << " ";
	...
	while (it1 != prj.GetAtomsEnd())
	{
		...
		file << (* it1).formal_charge << " ";		// signed ints; SHOW THE SIGN!!!
		file.unsetf(ios::showpos);
		
		i32u tmpflags = (* it1).flags & 0xFF;
		file << tmpflags << " ";			// unsigned ints; limited on range 0 <-> 255.
